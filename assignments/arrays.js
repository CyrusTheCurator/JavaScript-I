...
console.log(
  `Car ${inventory[32].id} is a ${inventory[32].car_year} ${
...
let lastCar = inventory[Array.length - 1];
console.log(
  `The last car in our inventory is a(n) ${lastCar.car_make}, ${
    lastCar.car_model
  }`
);
...
let modelSortedInventory = inventory.sort(function(a, b) {
...
modelSortedInventory.forEach(function(item, i) {
  carModels.push(modelSortedInventory[i].car_model);
});
...
oldCars = carYears.sort(function(a, b) {
  return a - b;
});

oldcars = oldCars.slice(0, oldCars.indexOf(1999));
